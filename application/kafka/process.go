...
import (
	...
	"github.com/lucasti79/codepix-go/application/factory"
	appmodel "github.com/lucasti79/codepix-go/application/model"
...
func (k *KafkaProcessor) processTransaction(msg *ckafka.Message) error {
	...
	topic := fmt.Sprintf("bank%s", createdTransaction.PixKeyTo.Account.Bank.Code)
...
func (k *KafkaProcessor) confirmTransaction(transaction *appmodel.Transaction, transactionUseCase usecase.TransactionUseCase) error {
	...
	topic := fmt.Sprintf("bank%s", confirmedTransaction.AccountFrom.Bank.Code)
	transactionJson, err := transaction.ToJson()
	if err != nil {
		return err
	}
